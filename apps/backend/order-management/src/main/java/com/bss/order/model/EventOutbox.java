...
public class EventOutbox {
    ...
    /**
     * B-11 fix: this id is generated by the CALLER (see {@link #of}) instead of by Hibernate
     * at insert time, so the same value can be embedded in {@code payload} as {@code eventId}
     * *before* the row exists. Downstream (billing-service's dedup log) then keys off this
     * stable outbox-row id instead of the EventBridge envelope id, which is minted fresh on
     * every PutEvents call — including retries of the very same row — and therefore cannot be
     * used as a dedup key without risking duplicate invoices.
     */
    @Id
    private UUID id;
    ...
    public static EventOutbox of(UUID id, String aggregateType, UUID aggregateId,
