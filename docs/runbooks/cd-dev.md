...
| `AssumeRoleWithWebIdentity` vẫn bị từ chối dù Environment/variable đúng | Repo dùng **immutable subject**: `sub` thật là `repo:OWNER@<owner-id>/REPO@<repo-id>:environment:dev` (không phải tên repo) | `gh api repos/OWNER/REPO/actions/oidc/customization/sub` → chép `sub_claim_prefix` vào `github_extra_sub_prefixes` trong `environments/shared/terraform.tfvars`, `terraform apply` |
| `Runner không kết nối được API server` (bước "API server có với tới được không?") | `public_access_cidrs` của cluster chỉ cho IP nhà bạn; runner GitHub không có IP cố định | Dev: đặt `public_access_cidrs = ["0.0.0.0/0"]` (đúng CLAUDE.md §4 cho dev; API vẫn cần IAM + RBAC) rồi `terraform apply` ở `environments/dev`. Staging/prod: ADR-006 |
