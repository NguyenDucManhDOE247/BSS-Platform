...
output "github_deployer_role_arn" {
  description = "Convenience mirror of environments/shared's output — the role for the GitHub Environment `staging` (variable AWS_ROLE_ARN)."
  value       = data.terraform_remote_state.shared.outputs.deployer_role_arns["staging"]
}
