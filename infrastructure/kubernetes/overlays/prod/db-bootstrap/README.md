...
## Why this isn't part of `overlays/prod`'s regular `kustomization.yaml`

A `Job`'s pod template is immutable once created — a routine `kubectl apply -k overlays/prod` on
...
1. `environments/prod` has been `terraform apply`'d (needs RDS + the `db-bootstrap` IRSA role —
   see the `db-bootstrap` entry in `environments/prod/main.tf`'s `module "iam"` block).
...
```bash
kubectl apply -k infrastructure/kubernetes/overlays/prod/db-bootstrap
...
```bash
kubectl delete -k infrastructure/kubernetes/overlays/prod/db-bootstrap
```
