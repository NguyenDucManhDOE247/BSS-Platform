...
## Why this isn't part of `overlays/staging`'s regular `kustomization.yaml`

A `Job`'s pod template is immutable once created — a routine `kubectl apply -k overlays/staging` on
...
1. `environments/staging` has been `terraform apply`'d (needs RDS + the `db-bootstrap` IRSA role —
   see the `db-bootstrap` entry in `environments/staging/main.tf`'s `module "iam"` block).
...
```bash
kubectl apply -k infrastructure/kubernetes/overlays/staging/db-bootstrap
...
```bash
kubectl delete -k infrastructure/kubernetes/overlays/staging/db-bootstrap
```
